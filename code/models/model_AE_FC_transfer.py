...
from model_AE import AutoEncoder_01, AutoEncoder_02
import json
...
class AE_FC_02(L.LightningModule):
    ...
    def __init__(self, config: dict, dbg_print = False) -> None:
        ...
        if dbg_print:
            print(config)
        tmp_cfg = json.load(open("G:/Codes/LuttingerWard_from_ML/configs/confmod_AE_GF_2.json"))
        if "AE_Version" in config and int(config["AE_Version"]) == 2:
            self.GF_encoder = AutoEncoder_02.load_from_checkpoint(self.hparams['GF_AE_path'], config=tmp_cfg)
        elif "AE_Version" in config and int(config["AE_Version"]) == 1:
            self.GF_encoder = AutoEncoder_01.load_from_checkpoint(self.hparams['GF_AE_path'], config=tmp_cfg)
        else:
            print("AE_Version not found in config, defaulting to AE_Version 1")
            self.GF_encoder = AutoEncoder_01.load_from_checkpoint(self.hparams['GF_AE_path'], config=tmp_cfg)
        ...
        if dbg_print:
            print("SE:")
        tmp_cfg = json.load(open("G:/Codes/LuttingerWard_from_ML/configs/confmod_AE_SE_2.json"))

        if "AE_Version" in config and int(config["AE_Version"]) == 2:
            self.SE_encoder = AutoEncoder_02.load_from_checkpoint(self.hparams['SE_AE_path'], config=tmp_cfg)
        elif "AE_Version" in config and int(config["AE_Version"]) == 1:
...
class AE_FC_03(L.LightningModule):
    """Fully conencted network with pretrained autoencoders."""
    def __init__(self, config: dict, dbg_print = False) -> None:
        super().__init__()
        self.config = config
        ...
        if dbg_print:
            print("G:")
        tmp_cfg = json.load(open("G:/Codes/LuttingerWard_from_ML/configs/confmod_AE_GF_tmp.json"))
        self.GF_encoder = AutoEncoder_01.load_from_checkpoint(self.hparams['GF_AE_path'], config=tmp_cfg)
        self.GF_encoder.eval()
        self.GF_encoder.freeze()
        if dbg_print:
