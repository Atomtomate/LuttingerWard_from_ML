...
def main(args):
    ...
    for mode in ['gf','se']:
        if mode == 'gf':
            config_path = join(dirname(__file__),'../configs/confmod_AE_GF_cluster.json')
        else:
            config_path = join(dirname(__file__),'../configs/confmod_AE_SE_cluster.json')
        ...
        for AE_layers in [2,3]:
            config['n_layers'] = AE_layers
            for laten_dims in [13,14,15,16]:
                config['latent_dim'] = laten_dims
                model_version = neptune.init_model_version(model=f"LWAEP-AE{mode.upper()}",name=f"L{AE_layers}LD{laten_dims}",project="stobbe.julian/"+pr_name)
                ...
                early_stopping = EarlyStopping(monitor="val/loss",patience=30, stopping_threshold=1e-11, min_delta=2e-12)
                swa = StochasticWeightAveraging(swa_lrs=1e-8,annealing_epochs=40, swa_epoch_start=280)
