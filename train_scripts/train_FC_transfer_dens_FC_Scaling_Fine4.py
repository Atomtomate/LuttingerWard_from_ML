...
def main(args):
    ...
    config_path = join(dirname(__file__),'../configs/confmod_AE_FC_transfer_with_dens2.json')
    ...
    for FC_layers in [6,7,8,9,10]:
        for FC_dim in [300]:
            if FC_layers == 6 and FC_dim == 300:
                ...
                model = AE_FC_02.load_from_checkpoint(checkpoint_path="G:/Codes/LuttingerWard_from_ML/.neptune/AE_FC_FCDimScale_with_dens/LWAEP-425/checkpoints/last.ckpt",config=config)
                ...
                trainer.fit(model, datamodule=dataMod, ckpt_path="G:/Codes/LuttingerWard_from_ML/.neptune/AE_FC_FCDimScale_with_dens/LWAEP-425/checkpoints/last.ckpt",)
